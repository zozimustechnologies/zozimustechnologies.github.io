...
import {assert} from '../shims/assert.js';
...
export class GameOverPanel {
  ...
  /**
   * Draw restart button — ZT logo fades into a circular restart arrow.
   *
   * Frame 0  (~875 ms): Large "ZT" badge — the Zozimus Tech logo.
   * Frames 1–8: A ↺ arc sweeps around the badge while "ZT" shrinks to
   *             a small label inside the finished restart-button circle.
   */
  private drawRestartButton() {
    ...
    const w = dimensions.restartWidth;   // 36
    const h = dimensions.restartHeight;  // 32
    const cx = restartTargetX + w / 2;
    const cy = restartTargetY + h / 2;
    const r = Math.min(w, h) / 2 - 1;   // ~15
    const frame = this.currentFrame;
    const lastFrame = animConfig.frames.length;  // 8
    ...
    // Clear previous frame's button area.
    this.canvasCtx.clearRect(
        restartTargetX - 3, restartTargetY - 3, w + 6, h + 6);

    if (frame === 0) {
      // ── Phase 1: ZT logo badge ──────────────────────────────────────────
      this.canvasCtx.beginPath();
      this.canvasCtx.arc(cx, cy, r + 1, 0, Math.PI * 2);
      ...
      this.canvasCtx.fillText('ZT', cx, cy);
    } else {
      // ── Phase 2: Animate to circular restart button ─────────────────────
      // progress goes 0 → 1 across frames 1–lastFrame.
      const progress = Math.min((frame - 1) / (lastFrame - 1), 1);

      // Background circle.
      this.canvasCtx.beginPath();
      this.canvasCtx.arc(cx, cy, r, 0, Math.PI * 2);
      this.canvasCtx.fillStyle = '#535353';
      this.canvasCtx.fill();

      // Sweeping arc (restart arrow indicator).
      const arcSpan = progress * (Math.PI * 1.75);  // grows to 315°
      const startAngle = -Math.PI / 2;
      const endAngle = startAngle + arcSpan;
      const arcR = r - 3;

      this.canvasCtx.beginPath();
      this.canvasCtx.arc(cx, cy, arcR, startAngle, endAngle);
      this.canvasCtx.strokeStyle = '#ffffff';
      this.canvasCtx.lineWidth = 2.5;
      this.canvasCtx.lineCap = 'round';
      this.canvasCtx.stroke();

      // Arrow-head tip — appears in the last 30 % of the animation.
      if (progress > 0.7) {
        const tipProgress = (progress - 0.7) / 0.3;
        const tipX = cx + Math.cos(endAngle) * arcR;
        const tipY = cy + Math.sin(endAngle) * arcR;
        const arrowSize = 3.5 * tipProgress;

        this.canvasCtx.save();
        this.canvasCtx.translate(tipX, tipY);
        this.canvasCtx.rotate(endAngle + Math.PI / 2);
        this.canvasCtx.beginPath();
        this.canvasCtx.moveTo(0, -arrowSize);
        this.canvasCtx.lineTo(arrowSize, arrowSize);
        this.canvasCtx.lineTo(-arrowSize, arrowSize);
      ...
      // "ZT" label — shrinks from 13 px → 9 px as animation completes.
      const textSize = Math.round(13 - 4 * progress);
      this.canvasCtx.fillStyle = '#ffffff';
      this.canvasCtx.font = `bold ${textSize}px monospace`;
      this.canvasCtx.textAlign = 'center';
      this.canvasCtx.textBaseline = 'middle';
      this.canvasCtx.fillText('ZT', cx, cy);
    }
