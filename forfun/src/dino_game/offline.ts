...
import {loadTimeData} from '../shims/load_time_data.js';

import {HIDDEN_CLASS} from '../neterror_constants.js';
...
/** Resets the singleton so initializeInstance can be called again (SPA re-navigation). */
export function resetInstance() { runnerInstance = null; }

/** Stops the RAF loop and destroys the singleton (call on component unmount). */
...
export class Runner implements ImageSpriteProvider, GameStateProvider,
  ...
  private loadSounds() {
    ...
    for (const sound in RunnerSounds) {
      ...
      assert(audioElement);
      const rawSrc = audioElement.getAttribute('src') || '';
      const soundKey = sound as keyof typeof RunnerSounds;

      // Standalone mode: src is a URL — fetch it as an ArrayBuffer.
      // Original Chrome mode: src is a data URI — decode the base64 part.
      if (rawSrc.startsWith('data:')) {
        const base64 = rawSrc.substr(rawSrc.indexOf(',') + 1);
        const buffer = decodeBase64ToArrayBuffer(base64);
        this.audioContext.decodeAudioData(buffer, audioBuffer => {
          this.soundFx = {...this.soundFx, [soundKey]: audioBuffer};
        });
      } else if (rawSrc) {
        fetch(rawSrc)
            .then(r => r.arrayBuffer())
            .then(
                buffer => this.audioContext!.decodeAudioData(
                    buffer, audioBuffer => {
                      this.soundFx = {
                        ...this.soundFx,
                        [soundKey]: audioBuffer,
                      };
                    }))
            .catch(() => { /* sound unavailable — game still playable */ });
      }
    }
  }
